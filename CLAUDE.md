...
Feature Hub — a desktop app for managing work features/epics with linked Claude Code sessions, external links, files, tasks, and notes. It also ships a CLI (`fh`) and an MCP server (`fh-mcp`) for interacting with features from the terminal or from Claude Code sessions.
...
2. **`fh` CLI** (`src-tauri/src/bin/fh.rs`) — terminal tool to `start`/`resume`/`list` Claude sessions linked to features. Spawns `claude` as a child process with `--session-id`, `--mcp-config`, and `--add-dir` flags. Uses per-feature workspace directories under the storage's `workspaces/` folder.
3. **`fh-mcp` MCP server** (`src-tauri/src/bin/fh_mcp.rs`) — stdio-based MCP server that Claude Code connects to during sessions. Exposes tools for reading/writing features, tasks, notes, context, links, repositories, tags, plans, skills, and settings. Uses `rmcp` crate with `#[tool]` / `#[tool_router]` / `#[tool_handler]` macros. When scoped to a feature (`--feature` flag), pre-loads a rich context snapshot into `ServerInfo.instructions`.
...
- `sessions` — Claude Code sessions with claude_session_id, timing (started_at/ended_at), project_path, branch
...
- `context` — one persistent context per feature (instructions/requirements that persist across Claude sessions)
- `plans` — Claude implementation plan submissions with status (pending/approved/rejected)
...
- `claude::launcher` — spawns `claude` CLI with session/MCP/directory args for start and resume
- `claude::scanner` — scans local `.claude/projects/` directories for Claude Code sessions, checks if sessions are active
- `claude::session_parser` — parses session JSONL transcripts and summary `.md` files for titles/summaries
...
Claude sessions can submit implementation plans via `submit_plan`. Plans have a status lifecycle: `pending` → `approved`/`rejected`. The UI shows plan cards in the AI tab where the user can review and approve/reject them. Claude sessions can poll `get_plan_status` to wait for approval before proceeding.
