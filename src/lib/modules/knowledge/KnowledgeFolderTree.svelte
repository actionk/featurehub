...
<div class="kb-tree">
  ...
  <div class="kb-tree-list">
    {#snippet folderNode(parentId: string | null, depth: number)}
      {#each childFolders(parentId) as folder (folder.id)}
        ...
        <div class="kb-tree-item kb-tree-folder" class:selected={isSelected} style="padding-left: {8 + depth * 16}px">
          <button class="kb-tree-expand" onclick={() => toggleExpand(folder.id)} aria-label="{isExpanded ? 'Collapse' : 'Expand'} folder">
          ...
          {#if renamingFolderId === folder.id}
            ...
            />
          {:else}
            <button class="kb-tree-label" onclick={() => onSelectFolder(folder.id)} ondblclick={() => startRename(folder.id, folder.name)}>
              {folder.name}
            </button>
          {/if}
        ...
        {#if isExpanded}
          {#each entriesInFolder(folder.id) as entry (entry.id)}
            <div class="kb-tree-item kb-tree-entry" class:selected={selectedEntryId === entry.id} style="padding-left: {24 + depth * 16}px" onclick={() => onSelectEntry(entry.id)} onkeydown={(e) => { if (e.key === 'Enter') onSelectEntry(entry.id); }} role="button" tabindex="0">
    ...
    {#each entriesInFolder(null) as entry (entry.id)}
      <div class="kb-tree-item kb-tree-entry" class:selected={selectedEntryId === entry.id} style="padding-left: 8px" onclick={() => onSelectEntry(entry.id)} onkeydown={(e) => { if (e.key === 'Enter') onSelectEntry(entry.id); }} role="button" tabindex="0">
        <span class="kb-entry-icon">📄</span>
        <span class="kb-tree-label">{entry.title || "Untitled"}</span>
      </div>
    {/each}
